...
for r in range(num_rounds):
    ...
    for i in range(num_participants):
        ...
        for name, parm in clients_models[i].named_parameters():
    ...
    print("Unmasking clients_models...")
    for i in range(num_participants):
        torch.manual_seed(seeds[i])
        np.random.seed(seeds[i])
        for name, parm in clients_models[0].named_parameters():
            mask = torch.rand(parm.size()) * (high - low) + low
            parm.data -= mask
    ...
    print('len(global_dict.keys())={}'.format(len(global_dict.keys())))
    # print('clients_models[0].state_dict()[conv1.weight][0]={}'.format(clients_models[0].state_dict()['conv1.weight'][0]))

    # for k in global_dict.keys():
    #     print('----------len(clients_models[0].state_dict()[{}])= {}'.format(k, len(clients_models[0].state_dict()[k])))
        # print('----------k={} client_models[0].state_dict()[{}].float()= {}'.format(k,k,clients_models[0].state_dict()[k].float()))
        # print(clients_models[0].state_dict()[k])
