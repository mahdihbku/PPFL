...
for N in Ns:
    for C in Cs:
        ...
        seed = random.getrandbits(RAND_BIT_SIZE)        # generating a seed
        masks_shapes = [parm.size() for parm in my_model.parameters()]
        masks = generate_masks_from_seed(seed, masks_shapes, rand_low, rand_high)      # generating masks from the seed
        mask_model(seed, my_model, rand_low, rand_high)     # masking the model
        masks_splits = split_additive_masks(masks, C, rand_low, rand_high)          # splitting the masked model      shape: P*n*mask
        masks_splits = transpose_list(masks_splits, axes=(1,0))             # splitting the masked model       shape: n*P*mask
        ...
        t1 = time.perf_counter()
        local_global_mask = sum_list_masks(transposed_clients_masks_splits[0])      # summing the shares
        ...
        t1 = time.perf_counter()
        global_masks = sum_list_masks(global_masks_splits)            # computing the global mask
        server_aggregate_masked(global_model, clients_models, global_masks)           # unmasking and computing global model
